...
void change_direction(vector<pair<int, pair<int, int>>>& pos, string& pieces, int idx){
	pos[pieces[idx] - 'A'].first ^= 1;
}
	...
	for(int turn = 1; turn <= 1000; turn++){
		...
		for(int i = 0; i < k; i++){
			...
			if(arr[next_r][next_c] == 2){
                if(!check){
                    change_direction(pos, board[r][c], idx);
                    i--;
                    check = true;
                }else{
                    check = false;
                ...
                reverse(board[r][c].begin() + idx, board[r][c].end());
            }
			board[next_r][next_c] += board[r][c].substr(idx);
            board[r][c] = board[r][c].substr(0, idx);
